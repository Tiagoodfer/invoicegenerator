...
import com.invoicegenerator.domain.InvoiceItem;
...
public class InvoicePDFService {
    ...
    public byte[] generateInvoicePDF(Invoice invoice) throws DocumentException, IOException {
        ...
        addCompany(document, invoice);
        addCommpanyClient(document, invoice);
    ...
    private void addCompany(Document document, Invoice invoice) throws DocumentException {
        Font normalFont = new Font(Font.FontFamily.HELVETICA, 10, Font.NORMAL);
        document.add(new Paragraph(invoice.getCompany().getName(), normalFont));

        Address recipientAddress = invoice.getCompany().getAddress();
        if (recipientAddress != null) {
            String addressLine = recipientAddress.getAddressOne();
            if (recipientAddress.getAddressTwo() != null && !recipientAddress.getAddressTwo().isEmpty()) {
                addressLine += ", " + recipientAddress.getAddressTwo();
            }
        ...
        document.add(new Paragraph(invoice.getCompany().getPhoneNumber(), normalFont));
        document.add(new Paragraph(invoice.getCompany().getEmail(), normalFont));
        document.add(new Paragraph("\n"));
    }

    private void addCommpanyClient(Document document, Invoice invoice) throws DocumentException {
        ...
        document.add(new Paragraph(invoice.getCompanyClient().getName(), normalFont));

        Address emitterAddress = invoice.getCompanyClient().getAddress();
        ...
        document.add(new Paragraph(invoice.getCompanyClient().getPhoneNumber(), normalFont));
        document.add(new Paragraph("\n"));
    }
    ...
    private void addSummaryTable(Document document, Invoice invoice) throws DocumentException {
        ...
        PdfPCell taxRateValue = new PdfPCell(new Phrase(invoice.getTax() + "%", normalFont));
    ...
    private void addCenteredRecipientDetails(Document document, Invoice invoice) throws DocumentException {
        ...
        Paragraph recipientDetails = new Paragraph(
                String.format("%s | %s | %s | %s",
                        ...
                        addressDetails,
                        invoice.getCompany().getPhoneNumber(),
